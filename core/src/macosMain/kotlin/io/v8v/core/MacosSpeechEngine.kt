...
/**
 * macOS implementation of [SpeechRecognitionEngine] using the Speech framework.
 *
 * Uses [SFSpeechRecognizer] for speech-to-text and [AVAudioEngine] for audio input.
 * Unlike iOS, macOS does not use AVAudioSession for audio routing — the
 * default audio device is used directly.
 *
...
class MacosSpeechEngine : SpeechRecognitionEngine {
    ...
    override fun startListening(language: String) {
        ...
        // Use on-device recognition if available (avoids network dependency)
        ...
        inputNode.installTapOnBus(0u, bufferSize = 4096u, format = recordingFormat) { buffer, _ ->
            if (buffer != null) {
                ...
                // Calculate RMS from the first channel for audio level
    ...
    override fun stopListening() {
        // Guard against re-entrant calls (e.g., cancel fires callback which calls stop again)
